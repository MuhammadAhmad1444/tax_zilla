...
const parseNumber = (value) => {
  ...
  // VAT calculations are not defined for negative money/rate in this UI.
  if (n < 0) return null;
  return n;
};
...
const computeVat = ({ net, tax, gross, rate }) => {
  ...
  // Pair 2: net + tax
    ...
    if (net === 0) {
      errors.push('Cannot compute VAT rate because Net Price is 0.');
  ...
  // Pair 4: tax + rate
    ...
    if (r === 0) {
      if (tax === 0) {
        errors.push('VAT Rate is 0%, so Net Price and Gross Price cannot be determined from Tax Amount alone.');
        computed = { net: null, tax, gross: null, rate };
      } else {
        errors.push('VAT Rate is 0%, but Tax Amount is not 0. Check inputs.');
        computed = { net: null, tax, gross: null, rate };
      }
    } else {
      const netCalc = tax / r;
  ...
  // Pair 5: tax + gross
    ...
    if (netCalc === 0) {
      errors.push('Cannot compute VAT rate because Net Price becomes 0 (Gross - Tax = 0).');
  ...
  // Pair 6: gross + rate
    ...
    const denom = 1 + r;
    if (denom === 0) {
      errors.push('VAT Rate results in an invalid denominator for Net/Gross conversion.');
    ...
    errors.push('Enter at least two fields (Net, Tax, Gross, or VAT Rate) to calculate VAT.');
  }
  ...
  (['net', 'tax', 'gross', 'rate']).forEach((key) => {
    ...
    if (Math.abs(expected - actual) > tol + toSafeTolerance(actual, tol) * 0.0001) {
      inconsistencies.push(
        `${key === 'rate' ? 'VAT Rate (%)' : key === 'tax' ? 'Tax Amount' : key === 'gross' ? 'Gross Price' : 'Net Price'} does not match the computed value.`
      );
    }
  });
...
export function ValueAddedTaxCalculator() {
  ...
  const result = useMemo(() => {
    ...
    return computeVat(parsed);
  }, [parsed, providedCount]);

  return (
    <TaxCalculatorShell
      title="Value-added Tax (VAT) Calculator"
      subtitle="Enter any two values. The missing fields will be calculated using standard VAT formulas."
    >
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        <div className="lg:col-span-1">
          <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5">
            ...
            <div className="mb-4">
              <label className="block text-sm font-bold text-gray-800 mb-2">Net Price (Before VAT)</label>
              <input
                value={netStr}
            ...
            <div className="mb-4">
              <label className="block text-sm font-bold text-gray-800 mb-2">Tax Amount (VAT)</label>
              <input
                value={taxStr}
            ...
            <div className="mb-4">
              <label className="block text-sm font-bold text-gray-800 mb-2">Gross Price (Including VAT)</label>
              <input
                value={grossStr}
            ...
            <div className="mb-4">
              <label className="block text-sm font-bold text-gray-800 mb-2">VAT Rate (%)</label>
              <input
                value={rateStr}
            ...
            <div className="text-xs text-gray-500 mt-2 leading-relaxed">
              Tip: leave the other fields blank and enter any 2 values. Example: Net Price + VAT Rate, or Gross Price + Tax Amount.
            </div>
          ...
          {providedCount < 2 ? (
            <div className="rounded-2xl border border-dashed border-gray-300 bg-white p-6 text-gray-600">
              ...
              <div className="text-sm leading-relaxed">
                ...
                <span className="font-semibold text-gray-800">VAT Rate (%)</span>.
              </div>

              <div className="mt-4 rounded-xl border border-gray-100 bg-gray-50 p-4">
                <div className="text-sm font-bold text-gray-800 mb-2">Example</div>
                <div className="text-sm text-gray-700 leading-relaxed">
                  If <span className="font-semibold">Net Price = 10,000</span> and <span className="font-semibold">VAT Rate = 17%</span>:
                  <br />
              ...
              <div className="text-xs text-gray-500 mt-3 leading-relaxed">
                We use standard VAT formulas to calculate the remaining fields automatically.
              </div>
            </div>
          ) : null}
                ...
                <div className="overflow-x-auto">
                  <table className="paktax-table">
                    <tbody>
                      ...
                      <tr>
                        <th className="text-left">VAT Rate (%)</th>
                        <td className="font-extrabold text-[var(--color-gold)]">
